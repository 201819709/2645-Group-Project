...
// ===== PWM CONFIGURATION =====
// Configure PWM to use TIM4 Channel 1 for the red LED on PB6
PWM_cfg_t pwm_cfg = {
    .htim = &htim4,
...
// Configure PWM to use TIM4 Channel 3 for the green LED on PB8
PWM_cfg_t pwm_green_cfg = {
    .htim = &htim4,
    .channel = TIM_CHANNEL_3,
    .tick_freq_hz = 1000000,  // 1MHz timer clock (prescaler = 79 with 80MHz input)
    .min_freq_hz = 10,
...
static void LCD_PrintWrappedCentered(const char *text, uint16_t y,
                                     uint16_t max_width, uint8_t colour,
                                     uint8_t font_size, uint16_t line_spacing)
{
...
static void LCD_ShowStartupScreen(uint8_t background_colour, uint8_t text_colour)
{
    LCD_Fill_Buffer(background_colour);
    LCD_PrintWrappedCentered("PlayStation Mini", 50, 220, text_colour, 2, 8);
    LCD_PrintWrappedCentered("Device Initializing...", 100, 220, text_colour, 2, 8);
    LCD_Refresh(&cfg0);
}
...
int main(void)
{
    ...
    // Startup animation: keep the text on screen while flashing the background colours
    LCD_ShowStartupScreen(1, 0);
    HAL_Delay(500);
    LCD_ShowStartupScreen(3, 1);
    HAL_Delay(500);
    LCD_ShowStartupScreen(2, 1);
    HAL_Delay(500);
    LCD_ShowStartupScreen(0, 1);
    HAL_Delay(500);
    ...
    PWM_Init(&pwm_green_cfg);
    PWM_SetFreq(&pwm_green_cfg, 1000);
    PWM_SetDuty(&pwm_green_cfg, 50);  // Start at 50% brightness
